...
namespace KerbalAlarmClock
{
    ...
    public class KACSettings
    {
        ...
        public void Load()
        {
            try
            {
                MonoBehaviourExtended.LogFormatted("Loading Config");
                ...
                //MonoBehaviourExtended.LogFormatted("{0}",strTimeFormat);
                ...
                //MonoBehaviourExtended.LogFormatted("{0}",this.TimeFormat.ToString());
                if (configfile.GetValue<bool>("TimeAsUT", false) == true)
                {
                    MonoBehaviourExtended.LogFormatted("Forcing New Format");
                ...
                {
                    MonoBehaviourExtended.LogFormatted("Trying New Alarms file..."); 
                    LoadAlarms();
                }
                else
                {
                    ...
                    do
                    {
                        ...
                        MonoBehaviourExtended.LogFormatted(strAlarm);
                        if (strAlarm != "")
                        {
                ...
                MonoBehaviourExtended.LogFormatted("Config Loaded Successfully");
            }

            catch (Exception ex)
            {
                MonoBehaviourExtended.LogFormatted("Failed To Load Config");
        ...
        {
            MonoBehaviourExtended.LogFormatted("Saving Config");
            ...
            configfile.save();
            MonoBehaviourExtended.LogFormatted("Saved Config");

        }

        public void SaveLoadObjects()
        {
            MonoBehaviourExtended.LogFormatted("Saving Load Objects");
            KSP.IO.PluginConfiguration configfile = KSP.IO.PluginConfiguration.CreateForType<KerbalAlarmClock>();
            configfile.load();
            configfile.SetValue("LoadManNode", this.LoadManNode);
            configfile.SetValue("LoadVesselTarget", this.LoadVesselTarget);
            configfile.save();
            MonoBehaviourExtended.LogFormatted("Saved Load Objects");
        }

        public void SaveAlarms2()
        {
            MonoBehaviourExtended.LogFormatted("Saving Alarms");
            ...
            //And close the file
            tw.Close();
            MonoBehaviourExtended.LogFormatted("Saved Alarms");
        }

        public void SaveAlarms()
        {
            MonoBehaviourExtended.LogFormatted("Saving Alarms-v3");
